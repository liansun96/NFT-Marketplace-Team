import React from "react";
import { AiFillHeart } from "react-icons/ai";

const BrowserCard = ({ browser }) => {
  return (
    <div className=" bg-primary rounded-xl shadow-lg p-5">
      <div className="flex justify-between items-center gap-4">
        <div className="flex items-center gap-5">
          <div className="cursor-pointer">
            <img src={browser?.profile} alt="" />
          </div>
          <div className="flex-col">
            <h1 className="font-semibold text-heading_text text-sm md:text-lg hover:text-purple cursor-pointer">
              {browser?.title}
            </h1>
            <p className="text-sm text-body_text">
              Created by{" "}
              <span className="font-semibold text-heading_text hover:text-purple cursor-pointer">
                {browser?.name}
              </span>
            </p>
          </div>
        </div>
        <div className="group flex items-center gap-1 bg-info px-2 py-2 rounded-lg cursor-pointer">
          <div className="">
            <AiFillHeart className="text-icon text-lg group-hover:text-purple  transition duration-300" />
          </div>
          <p className="font-semibold text-heading_text text-sm">100</p>
        </div>
      </div>
      <div className="flex items-center gap-3 mt-5">
        <div className="">
          <img src={browser?.image1} className="md:w-[335px]" alt="" />
        </div>
        <div className="flex flex-col justify-between gap-3 items-center">
          <div className="flex items-center gap-3">
            <div>
              <img src={browser?.image2} className="md:w-[165px]" alt="" />
            </div>
            <div>
              <img src={browser?.image3} className="md:w-[165px]" alt="" />
            </div>
          </div>
          <div className="">
            <img src={browser?.image4} className="md:w-[340px]" alt="" />
          </div>
        </div>
      </div>
    </div>
  );
};

export default BrowserCard;
